...
* **Justificativa Técnica e Decisões de Design:**
  ...
  - **Onda Fantasma Dinâmica, Pontilhada e Sincronizada em Fita Deslizante:** Em vez de instanciar e calcular um segundo modelo biológico em segundo plano (o que dobraria o consumo de CPU de 8% para 16% e drenaria baterias em celulares), o usuário clica em `📸 Capturar`. O sistema clona o vetor de pontos atual (`ghost_buffer = Some(buffer.clone())`). A curva fantasma é renderizada pontilhada (`line_dash: [4, 4]`) em segundo plano com cor esmaecida calibrada (`rgba(..., 0.38)`). No modo Fita Deslizante (`Rolling`), o buffer fantasma desliza circularmente a cada nova amostra em perfeito sincronismo temporal com a onda ativa, permitindo comparar de forma visualmente fluida as alterações de morfologia e duração do potencial de ação.
