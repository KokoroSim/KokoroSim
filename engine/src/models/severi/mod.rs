...
impl SeveriCell {
    ...
    pub fn compute_rates(&mut self, voi: f64) {
        ...
        let u1 = -((a[7] + 41.8) - c[95]);
        let term1 = if u1.abs() < 1e-5 {
            0.02839 * 2.5 * (1.0 - u1 / 5.0)
        } else {
            (0.02839 * u1) / ((u1 / 2.5).exp() - 1.0)
        };

        let u2 = -((a[7] + 6.8) - c[95]);
        let term2 = if u2.abs() < 1e-5 {
            0.0849 * 4.8 * (1.0 - u2 / 9.6)
        } else {
            (0.0849 * u2) / ((u2 / 4.8).exp() - 1.0)
        };
        a[41] = term1 + term2;

        let u3 = (a[7] + 1.8) - c[95];
        a[48] = if u3.abs() < 1e-5 {
            0.01143 * 2.5 * (1.0 - u3 / 5.0)
        } else {
            (0.01143 * u3) / ((u3 / 2.5).exp() - 1.0)
        };
